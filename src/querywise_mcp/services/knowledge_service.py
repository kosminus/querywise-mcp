...
from querywise_mcp.services.embedding_service import embed_text, embeddings_available
...
async def import_document(
    ...
    # Decide once whether to embed; if no provider is configured, store chunks
    # without embeddings and let keyword search handle retrieval.
    ...
    for idx, chunk_text in enumerate(chunk_texts):
        ...
        if do_embed:
            try:
                ...
                # Stop retrying after the first failure to avoid log spam.
                logger.warning(
                    "Embedding failed for '%s' (%s); storing remaining chunks "
                    "without embeddings.",
                    document_title,
                    e,
                )
